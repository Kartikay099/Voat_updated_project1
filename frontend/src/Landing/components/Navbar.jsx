...
import { Power } from "lucide-react"; // Changed from LogIn to Power
...
const Navbar = () => {
  ...
  const handleTabClick = (tab) => {
    setActiveTab(tab);
    if (tab === "left") {
      navigate("/profile"); // Apply Job
    } else if (tab === "right") {
      navigate("/hire"); // Hire Now
    }
  };

  return (
    <nav className="navbar bg-[#0b52c0]">
...
>
  ...
  >
    ...
    >
      ...
      >
        <button className="text-white text-3xl mb-8" onClick={() => setMenuOpen(false)}>
          ×
        </button>
        ...
        <ul className="flex flex-col gap-4 text-white text-sm">
          ...
          {["Home", "Updates", "FAQs", "Support"].map((item) => (
            <li key={item} className="border border-white text-center rounded-full py-1 cursor-pointer hover:bg-blue-600">
              {item}
            </li>
          ))}
          <li
